...
Given('I am logged in as the admin user', async ({page}) => {
    await page.goto('/login');
    await page.evaluate(() => localStorage.clear());
...
When('I login as the admin user', async ({page}) => {
    await page.getByPlaceholder('Email').fill(process.env.TEST_USER_EMAIL!);
    await page.getByPlaceholder('Password').fill(process.env.TEST_USER_PASSWORD!);
    await page.getByRole('button', {name: 'Login'}).click();
});
...
When('I login with invalid credentials', async ({page}) => {
    await page.getByPlaceholder('Email').fill(process.env.TEST_WRONG_EMAIL!);
    await page.getByPlaceholder('Password').fill(process.env.TEST_WRONG_PASSWORD!);
    await page.getByRole('button', {name: 'Login'}).click();
});
